                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            @if (Auth()->user()->role_name === 'Agent' || Auth()->user()->role_name === 'Merchant')
                                <button type="submit" class="btn btn-danger shadow btn-xs me-1 " style="float: right;"
                                    data-toggle="modal" data-target="#add_account" id="add-account">
                                    {{ __('messages.Add') }}
                                </button>
                            @endif
                        </div>
    ...
    <script>
        ...
        $(function() {
            var table = $('#bankDataTable').DataTable({
                ...
                ajax: {
                    url: "{{ route('Account: View Merchant Account') }}",
                    data: function(d) {
                        d.bank = $('#bank').val()
                        d.status = $('#status').val()
                        d.search = $('#search').val()
