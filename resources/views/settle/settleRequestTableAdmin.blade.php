...
@section('content')
    ...
    <div class="modal custom-modal fade" id="aproved_form" role="dialog">
        <div class="modal-dialog modal-dialog-centered" style="min-width: 200px">
            <div class="modal-content">
                <div class="modal-body" style=" border: 2px solid; ">
                    <div class="form-header">
                        <h3>{{ __('messages.Approved') }}</h3>
                        <p>{{ __('messages.Are you sure want to Approved?') }}</p>
                    </div>
                    <div class="modal-btn delete-action">
                        <form action="{{ route('Settlement: Approve Settlement') }}" method="POST">
                            ...
                            <input type="hidden" id="user_a_type" name="userType">
                            <div class="row">
                                <div class="col-md-12">
                            ...
                            <div class="d-flex justify-content-end mt-3">
                                <div class="d-inline-flex p-2">
                                    <button type="submit"
                                        class="btn btn-primary shadow btn-xs">{{ __('messages.Approve') }}</button>
                                </div>
                                <div class="d-inline-flex p-2">
    ...
    <div class="modal custom-modal fade" id="reject_form" role="dialog">
        <div class="modal-dialog modal-dialog-centered" style="min-width: 200px">
            <div class="modal-content">
                <div class="modal-body" style=" border: 2px solid; ">
                    <div class="form-header">
                        <h3>{{ __('messages.Rejected') }}</h3>
                        <p>{{ __('messages.Are you sure want to Rejected?') }}</p>
                    </div>
                    <div class="modal-btn delete-action">
                        <form action="{{ route('Settlement: Approve Settlement') }}" method="POST">
                            @csrf
                            <input type="hidden" id="settlement_e_id" name="settlement_id">
                            <input type="hidden" name="status" value="rejected">
                            <input type="hidden" id="user_e_type" name="userType">
                            <div class="row">
                                <div class="col-md-12">
                                    <textarea class="form-control" placeholder="{{ __('messages.Remark') }}" name="remark" id="" cols="30"
                                        rows="10"></textarea>
                            ...
                            <div class="d-flex justify-content-end mt-3">
                                <div class="d-inline-flex p-2">
                                    <button type="submit"
                                        class="btn btn-primary shadow btn-xs">{{ __('messages.Reject') }}</button>
                                </div>
                                <div class="d-inline-flex p-2">
                                    <a href="javascript:void(0);" data-dismiss="modal"
                                        class="btn btn-danger shadow btn-xs">{{ __('messages.Cancel') }}</a>
    ...
    @section('script')
        <script>
            $(document).on('click', '.status_settle', function() {
                $('#settlement_a_id').val($(this).data('id'));
                $('#settlement_e_id').val($(this).data('id'));
                $('#user_a_type').val($(this).data('type'));
                $('#user_e_type').val($(this).data('type'));
            });
            ...
            $(function () {
                var table = $('#requestDataTable').DataTable({
                    ...
                    order: [[ 0, "desc" ]],
                    ajax: {
                        url: "{{ route('Settlement: Settle Request View Settlement') }}",
                        data: function (d) {
